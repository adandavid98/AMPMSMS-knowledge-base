...
from ingestion import PDFParser, TextChunker, parse_document
...
async def submit_feedback(
    ...
    # Log score to Langfuse if trace_id is present
    if request.trace_id:
        score_val = 1.0 if feedback_type in ["thumbs_up", "resolved"] else 0.0
        telemetry.log_score(
            trace_id=request.trace_id,
            name="technician_feedback",
            value=score_val,
            comment=request.notes or feedback_type
        )
...
async def chat(
    ...
    x_cerebras_api_key: Optional[str] = Header(None, alias="X-Cerebras-Api-Key"),
    ...
    elif provider == "cerebras" and x_cerebras_api_key:
        custom_api_key = x_cerebras_api_key
    ...
    try:
        ...
        for upload_file in files:
            ...
            try:
                ...
                pages = parse_document(str(file_path), use_docling=config.USE_DOCLING_PARSER)
                if not pages:
